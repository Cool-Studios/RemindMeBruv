...
def start(bot, update):
    ...
    update.message.reply_text('Hello there. Please Press /help for more info')
...
def remember(bot, update, args, job_queue, chat_data):
    ...
    desc = ""
    if len(args) < 2:
        update.message.reply_text("Please don't forget that i don't know all the info\n"
    ...
    if dateparser(date) != -1 and timeparser(time) != -1:
        ...
        update.message.reply_text("❌ Hey, Time or Date format is wrong... are you drunk? \n"
...
def alarm(bot, job):
    ...
    bot.send_message(job.context, text='✔ Hey you have something to in half an hour... '
...
def set_timer(update, args, job_queue, chat_id, chat_data):
    ...
    try:
        ...
        update.message.reply_text('I got some problem setting the alarm! '
...
def forget(bot, update, args, chat_data):
    ...
    if isinstance(args, list):
        ...
        if len(args) == 0:
            update.message.reply_text("Please insert the note that you want me to delete!"
                                      "/forget <codEvent>")
        else:
            conn = sqlite3.connect('todo.db')
            c = conn.cursor()
...
def free(bot, update, chat_data):
    ...
    update.message.reply_text("I'm going on Holiday! Im finally free to go... 😎")


def default(bot, update):
    """Echo the user message."""
    update.message.reply_text("yeah... you are right!")
...
def schedule(bot, update, args):
    ...
    codu = getuser(chat_id)
    if codu == -1:
        update.message.reply_text("I think there is some kind of problem with me...\n"
    ...
    if len(data) != 0:
        ...
        if len(args) != 1:
            if len(args) == 0:
                # daily schedule
                update.message.reply_text("😵 Your daily appointment are:\n\n")
                for row in data:
                ...
                # daily schedule
                update.message.reply_text("👮 I think you wrote something wrong...\n\n")
        else:
            if args[0].find("/") != -1 or args[0].find("-") != -1:
                # args is a date -> schedule of that day
                update.message.reply_text("😵 Your appointments for " + args[0] + "are:\n\n")
                for row in data:
                    if dateparser(row[3]) != -1 and dateparser(row[3]) == dateparser(args[0]):
                        appointments += ("📌 " + print_appointment(row) + "\n")
                    else:
                        update.message.reply_text("😵 Wrong data format! I cant help you \n\n")
                update.message.reply_text(appointments + "\n")
            elif args[0] == "all":
                # args is all -> all my schedule
                update.message.reply_text("😵 Your appointments scheduled till now are:\n\n")
                for row in data:
                    appointments += ("📌 " + print_appointment(row) + "\n")
                update.message.reply_text(appointments + "\n")
            else:
        ...
        update.message.reply_text("I don't remember anything for today! So take it easy and relax 🎉 \n\n")


def own(bot, update):
    update.message.reply_text("Mucci is the best!")
